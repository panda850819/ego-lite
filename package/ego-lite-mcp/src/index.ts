...
export async function handleMcpRequest(
  ...
  if (request.method === 'tools/call') {
    ...
    try {
      ...
      return success(id, completeResult({ content: [{ type: 'text', text: JSON.stringify(data) }] }, modern));
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      return success(id, completeResult({ content: [{ type: 'text', text: message }], isError: true }, modern));
    }
  }
